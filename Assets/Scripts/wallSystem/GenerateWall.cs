...
namespace wallSystem
{
    public class GenerateWall : MonoBehaviour
    {
        ...
        private void Start()
        {
            var obj = Instantiate(Generator, new Vector3(transform.position.x, 0, transform.position.z),
                Quaternion.identity);
        ...
        private void GenerateTileFloor()
        {
            if (MakeFloor)
            {
                ...
                var numSides = E.Get().CurrTrial.enclosure.GroundTileSides == 0
                    ? 4
                    : E.Get().CurrTrial.enclosure.GroundTileSides;
                var tileSize = E.Get().CurrTrial.enclosure.GroundTileSize == 0.0
                    ? 1.0
                    : E.Get().CurrTrial.enclosure.GroundTileSize;
                var col = Data.GetColour(E.Get().CurrTrial.enclosure.GroundColor);
                var mesh = ConstructTileMesh(numSides, tileSize);
        ...
        private void GenerateWalls()
        {
            ...
            GameObject.Find("Ground").transform.position = new Vector3(E.Get().CurrTrial.enclosure.Position[0], 0,
            ...
            for (var i = 0; i < E.Get().CurrTrial.enclosure.Sides; i++)
            {
                ...
                var x = Cos(currentAngle) * E.Get().CurrTrial.enclosure.Radius +
                        E.Get().CurrTrial.enclosure.Position[0];
                var y = Sin(currentAngle) * E.Get().CurrTrial.enclosure.Radius +
                        E.Get().CurrTrial.enclosure.Position[1];
