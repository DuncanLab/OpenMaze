...
namespace main
{
    ...
    public class Loader : MonoBehaviour
    {
        ...
        public static void LogFirst()
        {
            ...
            {
                writer.Write(
                    "Timestamp, Trial Increment, Trial Number, X, Y, Angle, Environment Type, Sides, TargetFound, PickupType, " +
            ...
            if (_timer > 1f / (DS.GetData().OutputTimesPerSecond == 0 ? 1000 : DS.GetData().OutputTimesPerSecond) || targetFound == 1)
            {
                using (var writer = new StreamWriter("Assets/OutputFiles~/" + DS.GetData().OutputFile, true))
                {
                    ...
                    var str = string.Format(
                        ...
                        DateTime.Now.ToString("MM/dd/yyyy hh:mm:ss.fff tt"), data.DataSingleton.GetData().TrialInitialValue, s.TrialNumber, t.position.x, t.position.z, t.eulerAngles.y, s.EnvironmentType, s.CurrentMazeName,
