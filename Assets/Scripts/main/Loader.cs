...
namespace main
{
    ...
    public class Loader : MonoBehaviour
    {
        ...
        public static void LogFirst()
        {
            ...
            {
                writer.Write(
                    "Trial Increment, Trial Number, Time Since Experiement Start (Milliseconds), Time Since Trial Start (Milliseconds), X, Y, Angle, Environment Type, Sides, TargetFound, PickupType, " +
            ...
            if (_timer > 1f / (DS.GetData().OutputTimesPerSecond == 0 ? 1000 : DS.GetData().OutputTimesPerSecond) || targetFound == 1)
            {
                using (var writer = new StreamWriter("Assets/OutputFiles~/" + DS.GetData().OutputFile, true))
                {
                    ...
                    var str = string.Format(
                        ...
                        data.DataSingleton.GetData().TrialInitialValue, s.TrialNumber, timeSinceExperimentStart, timeSinceTrialStart, t.position.x, t.position.z, t.eulerAngles.y, s.EnvironmentType, s.CurrentMazeName,
